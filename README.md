...
`bevy-force-directed-graph` visualized graphs using a [force-directed graph drawing algorithm](https://en.wikipedia.org/wiki/Force-directed_graph_drawing) using the Bevy game engine. It simulates the nodes and edges based on physical forces (i.e. nodes are springs, nodes repulse each other) and user input (i.e. drag and drop).
...
* locked (bool), true if forces/inertia must not move a node when it is dragged by the user
...
time step dt | Assumed to be constant and thus **irrelevant**
-|-
...
-|-
Link force | Simulates edges as springs with a given target distance.
